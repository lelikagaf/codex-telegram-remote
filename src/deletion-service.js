...
function normalizeLocalTargets(values, { protectedPaths = [] } = {}) {
  ...
  const protectedKeys = new Set(
    defaultProtectedLocalPaths(protectedPaths).map(windowsPathKey),
  );
  const targets = [];
  const seen = new Set();
  for (const rawPath of rawPaths) {
    ...
    const key = windowsPathKey(resolved);
    if (key === windowsPathKey(root)) {
...
function inspectLocalTarget(target) {
  if (!fs.existsSync(target)) return { path: target, missing: true, files: 0, directories: 0, bytes: 0 };
...
function deleteLocalPaths(paths, options = {}) {
  ...
  const failed = inspected.filter((item) => fs.existsSync(item.path));
...
const REMOTE_DELETE_SCRIPT = [
  ...
  " if not p.startswith('/') or p in protected: raise RuntimeError('protected or relative path: '+p)",
...
function deleteRemotePaths({ host, user, port = 22, identityFile = null, paths }, {
  ...
  if (!/^[a-zA-Z0-9._:[\]-]+$/.test(safeHost)) throw new Error("Некорректный SSH host.");
  if (!/^[a-zA-Z0-9._-]+$/.test(safeUser)) throw new Error("Некорректный SSH user.");
